...
let _vue = new Vue({
  ...
  methods: {
        ...
        let newPage = contentSource.source === 'github'
          ? contentSource.isGhp
            ...
            : IS_JUNCTURE
              ...
              : path
          : path
        if (this.qargs.ref) newPage += `?ref=${this.qargs.ref || REF}`
        console.log(`load-page: path=${path} newPage=${newPage}`)
    ...
    */
    ...
    async parseEssay() {
      ...
      if (essayConfig.layout === 'vtl' || essayConfig.layout === 'vertical') document.getElementById('app').classList.add('vertical')
    },
  ...
  watch: {
    ...
    active(current, prior) {
      ...
      let activeSegment = app ? app.querySelector(`[data-id="${current}"]`) : null
      if (activeSegment) {
      ...
      let priorSegment = app ? app.querySelector(`[data-id="${prior}"]`) : null
      if (priorSegment) {
    ...
    hoverItem (eid) {
      ...
      if (app) {
        ...
        app.querySelectorAll(`[data-eid="${eid}"]`).forEach(el => el.classList.add('hover'))
      }
    }
...
async function getSiteConfig() {
  ...
  if (config.favicon) config.favicon = convertURL(config.favicon)
...
async function getContentSource() {
  ...
  contentSource = ghRepoInfo
    ? {
        ...
        ...{
          source: 'github',
          isGhp: location.host.indexOf('github.io') > 0,
...
async function dir(root, ghSource) {
  ...
  if (!dirCache[cacheKey]) {
    ...
    if (Object.keys(ghSource).length) {
      ...
      if (url) {
        _dirList = await ghDirList(url)
        if (_dirList) files = Object.fromEntries(_dirList.tree.map(item => [item.path, `https://raw.githubusercontent.com/${ghSource.acct}/${ghSource.repo}/${ghSource.hash || ghSource.ref}${root}/${item.path}`]))
      }
    }
...
function setMetaDescription(description) {
  let app = document.getElementById('app')
  let existing = app ? Array.from(app.querySelectorAll('meta[name=description]')).find(item => item) : null
